...
3. **The fall pipeline** — this is **Tutorial 501** (Simple Physics) end to end. Each object is a physics actor (`Simulate Physics` checked) tipped out of the trailer with a small `Add Impulse` toward the glass; gravity and tumbling come with the checkbox. Fab props need simple collision added first — 501 walks through it. The slow motion lives here too, tuned per actor: raise each falling body's `Linear Damping` — 501's dial for a dreamlike, floaty fall — and it drifts down as if through water, while everything else keeps its own pace, so the memories float while the creature gallops. Tutorial 4 is the conductor — not its array of scene objects, but its chain of timed, sequential events deciding when each object drops (501's own deviation list points to exactly this pairing). And the splat fires on the *impact itself*: **`OnComponentHit`** is the event for a physics object striking something — the falling memory hits the glass and triggers its image, no separate trigger zone needed. Fine-tuning the feel comes in step 11.
...
11. **Second pass on physics** — dial in the rest of Tutorial 501's properties: `Mass` so heavier memories fall with more weight, `Restitution` on a Physical Material for how they bounce or land dead against the glass, and impulse variation so they tumble believably and land reliably in slow motion. The only off-map piece here is runtime spawning to replace pre-placed batches.

### Off-Map

These are the features you'll research on your own, outside the bundle — and an honest heads-up: this project still sits further off-map than most. The tutorials cover real pieces — the triggers, the materials, the particles, the falling-and-bouncing physics (Tutorial 501), the creature's animations — but the systems that make this piece *itself* — the drive rig, the chasing truck, the mix — are Blueprint scripting beyond what the tutorials assume. Taking this idea on means deliberately growing your Blueprint skills past the beginner patterns. That's worth knowing as you commit; it's not a reason to shrink the idea necessarily. Each item below has a first pass within reach:
