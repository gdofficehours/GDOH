...
*An example of how the Companion maps a student's project. The student brought a single-role Investigator idea; this is the response. Use it as a model for tone, structure, and how much to say — not as a script to copy.*

**Dear student: this is not here to demonstrate a quality idea, to influence the kind of idea you devise.  It is here so that the LLM has a structural reference - a key component of how context engineering works.**
...
1. **Get a first-person player character placed and walking.** The Unreal first-person template gives you the POV this idea wants.  You could incorporate Tutorial **202** to make the player avatar a MetaHuman, but that's a bit ambitious for The Investigator project, for its mild payoff.  The first-person default is the cleanest fit for a looking-around experience.
2. **Import aquarium models — mostly from Fab.** Tanks, benches, the kiosks, broken equipment, the whale, the floating globe, signage. Fab (Epic's marketplace) has most of these as free assets. Stage with placeholder boxes first to feel the route — how far from the entrance to the whale tank, where the eye lands first — then dress it in.
3. **Tutorial 801 — Inspect an Object.** This is the **heart of your project** — the Investigator's primary tutorial. The coded paper is a readable note (full-screen widget, player input locked while it's open). The broken equipment, the signage — every examinable in the building is a deviation of this one pattern: same inspect logic, different content.
...
5. **Tutorial 4 — the kiosks.** Tutorial 4's spatial ambient sound plays a clip when the player comes near a placed actor — exactly your kiosks. Each one gets its ecosystem audio on proximity, no button press: walk close, the hopeful narration starts.
6. **Tutorial 4 again — the approach to your ending.** As the player nears the last corner, the two voices and the walkie static fade in as spatial ambient sound — you feel them before you see them. The player turns the corner, the two figures are there for a moment (a Mixamo character standing in place is enough), and a trigger cuts to black. Tutorial 4 stages all of it — the ambient voices and the sequential triggers — and the cut itself is a simple fade. The two refinements you want — firing the cut *only* when the player is looking right at the figures, and the line of text over the black — reach past the tutorials; see *Outside the vault* just below.
7. **Tutorial 1's basic lighting** to keep everything readable. The colder, moodier aquarium atmosphere — the blue underwater glow, heavier mood-lighting — is more a **Traveler** strength, the role you work in next. Get it clean and readable now; the mood pass comes later.
...
9. **A Timeline to make the globe bob.** To sell the globe *floating* rather than fixed in place, drive its up-and-down motion with a **Timeline** — the same Timeline that opens the door in Tutorial 1, just set to loop. A deliberate compromise worth taking: true physics buoyancy would be more convincing, but physics simulation is off-map, and a gentle looping bob gets you *floating* for a fraction of the trouble.
10. **Tutorial 2's key + Tutorial 1's door** — You didn’t say that the player would use the key or keypad, but if you wanted to create such mechanics, Tutorial 2 gives you a key the player picks up and uses; Tutorial 1 makes a door slide open when triggered.  You could script logic – with a boolean – for the door’s trigger to work upon the boolean being true.  But in a project of this scope, you likely will get away with the door opening regardless, as this not core to the experience. Also, I think you meant the code on the paper to be just a clue to read — good, creating a functional keypad is too ambitious for The Invesigator project.  

### Outside the vault

- **Making the big tanks read as genuinely underwater.** Tutorial 302 gives you gorgeous water seen from *above* — focused on the surface. But the player meets these tanks from *below*, looking up, with the surface overhead, and from that angle 302's richness mostly falls away. Selling a convincing underwater look from beneath usually takes floating bubbles and a refraction/distortion pass. The bubbles you can approach with Niagara (the ambient-particle pattern in Tutorial **702**); the refraction look itself is something you'd research outside the vault (a search like *"Unreal underwater post process"* is the start). Worth a quick experiment on one tank before you commit the whole space to it.  
...
- **The approach as a clock.** The distant talking and the walkie static mean someone is *coming.* Lean into it: the closer they get, the less time the player has — so every object becomes a choice between lingering on the whale's globe and pressing on before the encounter lands.
- **Whether to dwell in the hopeful version.** The kiosks earnestly sell *"we're in this together."* Dwelling there makes the globe's betrayal land like a punch — but dwelling is also time not spent moving, with someone approaching. The player trades the safety of pressing on for the depth of having believed.
...
**Who the player turns out to be — and what the ending says.** This is the meaning of the whole piece, and it's not a call the Companion should make for you. You named the gap yourself, and taking it to the instructor is exactly right. The one *structural* thing worth holding onto while you decide: a reveal only lands if it **recontextualizes what's already been seen.** A few directions, each of which rewrites the game:
