...
**Outcome.** By the end of this tutorial, the GET folder is open as a VSCode workspace, Gemini runs in VSCode's integrated terminal, and you can read tutorials and references in markdown preview while you talk to the GET.
...
- **Markdown preview** so you can read content the GET cites while you keep talking to it
...
2. The terminal opens **inside the workspace folder** — no `cd` needed. Confirm by checking the prompt; it should show the GET folder name.
3. Type:
   ```
   gemini
   ```
...
4. At Gemini's prompt, type:
   ...
   Start a GET session.
   ```
   The GET greets you and asks what you're working on, exactly as in [[Tutorial 1004 - Set Up The GET for Gemini|Tutorial 1004]].

---

## 4. Markdown preview (optional but useful)

Many GET files — tutorials, wiki pages, SPR pages, references — are markdown. VSCode's preview renders them like a webpage so you can read alongside the agent.
...
When the GET cites a wiki page or reference, click that file in the Explorer and preview it without leaving VSCode.
...
- Open the workspace from the command line by running `code .` inside the GET folder (after installing the `code` command via VSCode's command palette → "Shell Command: Install 'code' command in PATH")
...
- Use VSCode's built-in Git integration to pull future GET updates if you used `git clone` in [[Tutorial 1004 - Set Up The GET for Gemini|Tutorial 1004]] Chapter 1 — the Source Control panel handles `git pull` with one click
