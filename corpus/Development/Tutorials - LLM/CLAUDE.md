...
Setup tutorials: how a student gets an AI tool running and has their first GET conversation. These teach **no gameplay features** and must never appear in a build order — if you're tempted to cite one for a game mechanic, that's a wander and the feature is off-map. The published human-facing list is `index.md`; use that when you need the reading order.

## The organizing principle — quick-start-first

**Every path is a single self-contained quick start.** One page takes a student from nothing installed to a first GET conversation and a saved Prototype Map. No prerequisite chain, no terminal assumed, no GitHub account required.

This replaced (2026-07-19) an earlier shape: **per-tool four-step sequences** — install for the terminal → set up desktop → set up in VSCode → set up The GET — one tool spread across four pages. Those are retired.
...
- **1090 — Fork and Update The GET.** Optional GitHub setup, deliberately numbered high so it reads as *after*, not *before*.
- **1101 — Keep Using The GET.** What to do once the first session has happened.
...
- New pages carry `type: Tutorial` and `cssclasses: unreal-tutorial`. See `The-GET/scripts/README.md` for the corpus-wide conventions.
...
For drafting *Unreal* tutorials (a different voice and shape entirely) see `The-GET-dev/agent/tutorial-drafting.md`.
