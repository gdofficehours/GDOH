...
| `student-notes-private/` | **Your personal working folder.** The GET saves your Prototype Maps, notes, references you collect, and a "where we left off" summary here. Local-only — never pushed anywhere. See the README inside. |
...
- **Blueprint-only.** This course uses Unreal's visual scripting — no C++. The GET won't suggest C++ even if asked.
...
- **Push back on the AI.** If it's off-track, vague, or wrong, say so. It'll adjust. The GET is meant to be a collaborator, not an oracle.
